...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        # table
        self.tree = ttk.Treeview(self, column=('ID', 'name', 'author', 'type', 'category', 'reading_hall'), height=30,
                                 show='headings')
    ...
    def view_catalog(self):
        self.db_books.db_forms_conn.execute('''SELECT * FROM books''')
        [self.tree.delete(i) for i in self.tree.get_children()] # отображение на экране
        [self.tree.insert('', 'end', values=row) for row in self.db_books.db_forms_conn.fetchall()]

    def search_in_catalog(self, name):
        name = ('%' + name + '%',)
        self.db_books.db_forms_conn.execute('''SELECT * FROM books WHERE name LIKE ?''', name)
        [self.tree.delete(i) for i in self.tree.get_children()]
        [self.tree.insert('', 'end', values=row) for row in self.db_books.db_forms_conn.fetchall()]

    def search_among_authors(self, author):
        author = ('%' + author + '%',)
        self.db_books.db_forms_conn.execute('''SELECT * FROM books WHERE author LIKE ?''', author)
        [self.tree.delete(i) for i in self.tree.get_children()]
        [self.tree.insert('', 'end', values=row) for row in self.db_books.db_forms_conn.fetchall()]
...
class DataBaseForms:
    def __init__(self):
        self.db_forms = sqlite3.connect('books.db')
        self.db_forms_conn = self.db_forms.cursor()
        self.db_forms_conn.execute('''CREATE TABLE IF NOT EXISTS forms  (user_id integer primary key, name text, surname text, middle_name text, phone text, mail text)''')
        self.db_forms.commit()

    def insert_books(self, name, author, type, category, readinghall):
        self.db_forms_conn.execute('''INSERT INTO forms(name, surname, middle_name, phone, mail) VALUES (?, ?, ?, ?, ?)''',
                                   (name, author, type, category, readinghall))
        self.db_forms.commit()

if __name__ == "__main__":
    root = tk.Tk()
    db_form = DataBaseForms()
    app = Main(root)
    app.pack()
