...
open_librarian_page = False
open_reader_page = False
...
class Main(tk.Frame):
    def __init__(self, root):
        super().__init__(root)
        if open_librarian_page == False:
            if open_reader_page == False:
                self.init_main()
            else:
    ...
    def init_reader_page(self):
        ...
        # table
        self.tree = ttk.Treeview(self.tab_1, column=('ID', 'name', 'author', 'type', 'category', 'reading_hall'),
                                 height=30,
                                 show='headings')
